...
const CONFIG: &str = "jjzeron-update.json";
...
const BACKUP: &str = "jjzeron.exe.old";
...
const INCOMING: &str = ".jjzeron-update-incoming.exe";
...
pub(super) fn is_managed(exe: &Path) -> bool {
    exe.file_name().is_some_and(|name| name == "jjzeron.exe")
...
pub fn artifact(version: &str) -> String {
    format!("jjzeron-{version}-windows-{}.exe", std::env::consts::ARCH)
}
...
pub async fn stage(
    ...
    let temporary = tempfile::Builder::new()
        .prefix(".jjzeron-update-")
        .tempdir_in(directory)?;
    let staged = temporary.path().join("jjzeron.exe");
    ...
    .context("staged executable version check timed out")??;
        ...
        output.status.success()
            && String::from_utf8_lossy(&output.stdout).trim()
                == format!("jjzeron {}", manifest.version),
...
pub fn apply(staged: &Path, directory: &Path, relaunch: bool) -> anyhow::Result<()> {
    let installed = directory.join("jjzeron.exe");
    ensure!(
...
mod tests {
    ...
    /// A staged update layout: `<dir>/jjzeron.exe` plus its `sha256` sidecar.
    fn staged_with(dir: &Path, bytes: &[u8]) -> PathBuf {
        let staged = dir.join("jjzeron.exe");
    ...
    fn portable_install_requires_explicit_configuration() {
        ...
        let exe = dir.path().join("jjzeron.exe");
    ...
    fn failed_swap_after_old_image_moved_restores_the_installation() {
        ...
        let installed = install.path().join("jjzeron.exe");
