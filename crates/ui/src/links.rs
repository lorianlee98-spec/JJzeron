...
pub fn parse_zeron_conversation_link(url: &str) -> Result<ConversationDeepLink, &'static str> {
    let rest = url
        .strip_prefix("jjzeron://open/chat/")
        .ok_or("not a JJzeron conversation link")?;
...
mod tests {
    ...
    fn malformed_or_foreign_links_are_rejected() {
        ...
        assert!(parse_zeron_conversation_link("jjzeron://open/chat/id").is_err());
        assert!(parse_zeron_conversation_link("jjzeron://open/chat/%GG?workspace=x").is_err());
    }
