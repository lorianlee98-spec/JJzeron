//! `jjzeron daemon …` — install/manage `jjzeron headless` as a background service:
...
//! `ZERON_EDGE_URL=… jjzeron daemon install` bakes that override in.
//!
//! Auth is decoupled: without a saved session the service remains up on the
//! local-only profile. `jjzeron login` and a service restart opt into sync.
...
const LAUNCHD_LABEL: &str = "sh.jjzeron.app";
const SYSTEMD_UNIT: &str = "jjzeron.service";
...
pub fn install(data_dir: &Path) -> anyhow::Result<()> {
    let exe = std::env::current_exe().context("resolving the jjzeron executable path")?;
    ...
    if cfg!(target_os = "macos") {
        ...
        bail!("jjzeron daemon is only supported on macOS (launchd) and Linux (systemd)");
    }
    println!(
...
pub fn start() -> anyhow::Result<()> {
    if cfg!(target_os = "macos") {
        let plist = launchd_plist_path()?;
        if !plist.exists() {
            bail!("not installed — run `jjzeron daemon install` first");
        }
        ...
        if !output.status.success() {
            println!(
                ...
                if launchd_plist_path()?.exists() {
                    " (installed — `jjzeron daemon start`)"
                } else {
                    " (not installed — `jjzeron daemon install`)"
                }
            );
            return Ok(());
        }
...
fn render_systemd_unit(exe: &Path, env: &[(String, String)]) -> String {
    let mut unit = String::from(
        "[Unit]\nDescription=JJzeron headless engine\nAfter=network-online.target\nStartLimitIntervalSec=60\nStartLimitBurst=5\n\n[Service]\n",
    );
...
/// A managed exe under `~/.jjzeron/app/` resolves to its versioned directory;
/// point the service back at `app/current` so an update can relink it.
...
fn exec_path_for(exe: &Path, home: Option<&Path>) -> String {
    let installed = home
        .map(|home| home.join(".jjzeron/app"))
        .is_some_and(|app_root| exe.starts_with(app_root));
    if installed {
        "%h/.jjzeron/app/current/jjzeron".to_string()
    } else {
        format!("{}", exe.display())
    }
}
...
mod tests {
    ...
    fn systemd_unit_shape() {
        let unit = render_systemd_unit(
            Path::new("/usr/local/bin/jjzeron"),
            &[
        ...
        assert!(unit.contains("ExecStart=/usr/local/bin/jjzeron headless\n"));
        ...
        assert!(unit.contains("EnvironmentFile=-%h/.jjzeron/env"));
    ...
    fn installed_exe_uses_the_current_symlink() {
        ...
        assert_eq!(
            exec_path_for(
                Path::new("/home/u/.jjzeron/app/0.3.0/jjzeron"),
                Some(Path::new("/home/u")),
            ),
            "%h/.jjzeron/app/current/jjzeron"
        );
        // Source build: literal path.
        assert_eq!(
            exec_path_for(
                Path::new("/src/target/debug/jjzeron"),
                Some(Path::new("/home/u"))
            ),
            "/src/target/debug/jjzeron"
        );
    }
    ...
    fn launchd_plist_shape() {
        let plist = render_launchd_plist(
            Path::new("/Users/x/jjzeron & co/jjzeron"),
            &[("ZERON_EDGE_URL".into(), "https://e?a=1&b=2".into())],
            Path::new("/Users/x/.jjzeron/daemon.log"),
        );
        assert!(plist.contains("<key>Label</key><string>sh.jjzeron.app</string>"));
        // XML-escaped exe path and env value.
        assert!(plist.contains("<string>/Users/x/jjzeron &amp; co/jjzeron</string>"));
        ...
        assert!(
            plist.contains(
                "<key>StandardOutPath</key><string>/Users/x/.jjzeron/daemon.log</string>"
            )
        );
    }
}
