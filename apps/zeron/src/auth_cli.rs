//! `jjzeron login` / `jjzeron logout` / `jjzeron status` — the standalone auth surface.
//!
//! Sign-in used to live only inside `jjzeron headless`, coupling authentication to
//! the long-running daemon. These commands work on the persisted session
//! (`{data_dir}/session.json`) and exit, so a service-managed `jjzeron headless`
    ...
    match scope {
        ...
        WorkspaceScope::Synced => match auth {
            ...
            AuthState::NeedsOrganization { user } => AccountStatus {
                mode: "synced",
                auth: format!(
                    "signed in as {} but no workspace selected — run `jjzeron login`",
                    user.email
                ),
                healthy: false,
            },
            AuthState::SignedOut => AccountStatus {
                mode: "synced",
                auth: "saved session is no longer valid — run `jjzeron login`".into(),
                healthy: false,
            },
        },
    }
...
/// `jjzeron login`: authenticate via the paste-code flow (and workspace
...
pub async fn login(config: EngineConfig) -> anyhow::Result<()> {
    ...
    if let AuthState::SignedIn { user, org_id } = auth.state() {
        ...
        println!("Run `jjzeron logout` first to switch accounts.");
        println!("The next engine start will use the synced workspace.");
        return Ok(());
    }
    if !std::io::stdin().is_terminal() {
        anyhow::bail!("jjzeron login needs an interactive terminal");
    }
    ...
    match auth.state() {
        AuthState::SignedIn { user, org_id } => {
            ...
            println!(
                "Sync is ready. Start or restart JJzeron to open the synced workspace; existing local sessions will stay local."
            );
        }
...
/// `jjzeron logout`: remove the persisted session.
...
/// `jjzeron status`: report the fixed scope a new engine would select, optional
    ...
    InstanceLock::acquire(&config.data_dir).map_err(|err| {
        anyhow::anyhow!(
            "{err}\nCannot {verb} while an engine is running — stop it first \
             (`jjzeron daemon stop`, or quit the JJzeron app), or use the running UI instead."
        )
    })
}
